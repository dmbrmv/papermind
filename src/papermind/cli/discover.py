"""papermind discover CLI command — find papers from academic search APIs."""
...
def discover_cmd(
    ...
    ``kb/.papermind/last_search.json`` for use by the download command.
    """
    ...
    if not providers:
        console.print(
            ...
            "Configure API keys via env vars or .papermind/config.toml."
        )
        raise typer.Exit(code=1)
...
def _build_providers(source: str, cfg: PaperMindConfig) -> list:
    ...
    if source in ("all", "exa") and not cfg.exa_key:
        console.print(
            ...
            "set PAPERMIND_EXA_KEY or exa_key in config.toml."
        )
    return providers
...
def _cache_results(kb: Path, query: str, results: list) -> None:
    """Persist results to ``kb/.papermind/last_search.json``.
